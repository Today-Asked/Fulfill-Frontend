...
export function SearchPage() {
  ...
  async function toggle(id: number) { if (!user) { navigate('/login'); return; } try { const active = await toggleSaveCreator(user.id, id); setSaved((old) => { const next = new Set(old); active ? next.add(id) : next.delete(id); return next; }); } catch (e) { setError(e instanceof Error ? e.message : '無法更新候選名單。'); } }

  return <main className="mx-auto min-h-full w-full max-w-6xl px-5 pb-32 pt-10 lg:px-10 lg:pt-14">
    <div className="mb-8"><p className="mb-2 text-xs tracking-[0.18em] text-white/40">DISCOVER</p><h1 className="text-3xl font-semibold tracking-tight">找創作者</h1><p className="mt-2 text-sm text-white/45">依服務與接案狀態縮小範圍，再把適合的人加入候選名單。</p></div>
