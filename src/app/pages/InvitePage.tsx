...
import { createCommission, type CommissionDraft } from "../../lib/commissions";
...
export function InvitePage() {
  ...
  useEffect(() => { const id = Number(artistId); if (!Number.isSafeInteger(id)) { setError('無效的創作者連結。'); setLoading(false); return; } getCreator(id).then(setCreator).catch((e) => setError(e.message)).finally(() => setLoading(false)); }, [artistId]);
  ...
  async function submit(e: React.FormEvent) { e.preventDefault(); if (!user || !creator) return; if (creator.userId === user.id) { setError('不能邀請自己。'); return; } setBusy(true); setError(''); try { await createCommission(creator.artistId, user.id, draft); navigate('/orders'); } catch (err) { setError(err instanceof Error ? err.message : '邀請送出失敗。'); setBusy(false); } }
  ...
  return <main className="mx-auto min-h-full w-full max-w-3xl px-5 pb-32 pt-10 lg:pt-14">
    ...
    <h1 className="text-3xl font-semibold tracking-tight text-white">邀請 {creator?.name ?? '創作者'} 合作</h1>
    <p className="mt-3 text-sm leading-6 text-white/50">這不是付款訂單。創作者接受後才會開啟對話，請先把範圍、預算與時程說清楚。</p>
    {error && <div role="alert" className="mt-6 border-l-2 border-red-400 bg-red-400/10 px-4 py-3 text-sm text-red-100">{error}</div>}
    {!creator ? <p className="mt-8 text-white/50">找不到這位創作者，或對方尚未公開接案資料。</p> : <form onSubmit={submit} className="mt-8 grid gap-6">
      ...
      <button disabled={busy || creator.userId === user?.id} className="flex h-12 items-center justify-center gap-2 bg-white font-semibold text-black disabled:cursor-not-allowed disabled:opacity-40"><Send size={17} />{busy ? '送出中' : '送出合作邀請'}</button>
    </form>}
  </main>;
}
